...
export async function createProblem(data: CreateProblemFormData): Promise<Response<EntityID>> {
...
export async function getCreatedProblems(): Promise<Response<Pagination<ProblemListItem>>> {
	return fetchWithAuth(`${config.api.basepath}/creator/problems`, {
		method: 'GET',
	}, PaginationSchema(ProblemListItemSchema));
}

export async function getProblemByID(id: ID): Promise<Response<ProblemDetailed>> {
	return fetchWithAuth(`${config.api.basepath}/problems/${id}`, {
		method: 'GET',
	}, ProblemDetailedSchema);
}

export async function executeSolution(code: string): Promise<Response<ExecutionResult>> {
...
export async function getProblemSubmissions(contestID: ID, charcode: string, limit: number): Promise<Response<Pagination<Submission>>> {
