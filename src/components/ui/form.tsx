...
import { createContext, useContext, forwardRef, useId, HTMLAttributes, ElementRef, ComponentPropsWithoutRef } from "react";
...
const FormLabel = forwardRef<ElementRef<typeof LabelPrimitive.Root>, ComponentPropsWithoutRef<typeof LabelPrimitive.Root>>(({ className, ...props }, ref) => {
    ...
    return (
        <Label
            ref={ref}
            className={cn(error && "text-scarlet-500 dark:text-scarlet-900", className)}
            htmlFor={formItemId}
            {...props}
        />
    );
...
const FormMessage = forwardRef<HTMLParagraphElement, HTMLAttributes<HTMLParagraphElement>>(({ className, children, ...props }, ref) => {
    ...
    return (
        <p
            ref={ref}
            id={formMessageId}
            className={cn("text-[0.8rem] font-medium text-scarlet-500 dark:text-scarlet-900", className)}
            {...props}
        >
            {body}
        </p>
